			...
			switch (true) {
					case $this->request->is('get'):
						if(!isset($user_id) OR $user_id=="") {
							...
							$return_data= $this->Users->find('all',['contain' => ['UserFavLocations','Teams','FavouriteUsers','Games'=>['Users'=>['SportsPreferences'=>['Sports']],'Sports'],'SportsPreferences'=>['Sports']]])->select()->where(['Users.id'=>$user_id])->first();
							if($return_data) {
