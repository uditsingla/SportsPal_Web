							...
							if($user_id!="") {
								$return_data= $this->Users->find('all',['contain' => ['SportsPreferences']])->select()->where(['Users.id'=>$user_id])->first();
									if($return_data) {
										...
										$allGames = $this->Games->find('all',['contain' => ['Users'=>['SportsPreferences'], 'Sports', 'Teams']])->select()->where($whr)->order(['Games.id' => 'DESC']);
										...
										$allGames = $this->Games->find('all',['contain' => ['Users'=>['SportsPreferences'], 'Sports']])->select()->order(['Games.id' => 'DESC']);;
										$success = true;
									}
						...
						case $this->request->is('post'):
							...
							if($request_data['user_id']!="") {
								...
								if($return_data) {
									...
									$allGames = $this->Games->find('all',['contain' => ['Users'=>['SportsPreferences'], 'Sports']])->select()->where($whr)->order(['Games.id' => 'DESC']);
									/******************************************* Get NearBy *********************************/
			...
			switch (true) {
						case $this->request->is('get'):
								if($game_id!='') {
									...
									$allChallenge = $this->GameChallenges->find('all',['contain' => ['Teams'=>['Sports'], 'Users']])->where(['GameChallenges.game_id'=>$game_id])->order(['GameChallenges.id' => 'DESC']);
									$success = true;
								} else {
			...
			switch (true) {
						case $this->request->is('get'):
								if($user_id!='') {
									...
									$allChallenge = $this->Games->find('all',['contain' => ['GameChallenges'=>['Teams'=>['Sports'], 'Users'],'Sports','Teams']])->where(['Games.user_id'=>$user_id])->order(['Games.id' => 'DESC']);
